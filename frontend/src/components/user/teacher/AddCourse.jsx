...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { sections, ...courseData } = addCourse;

    // Append core course data
    for (let key in courseData) {
      formData.append(key, courseData[key]);
    }

    // Section metadata (titles + descriptions)
    const sectionMeta = sections.map(({ S_title, S_description }) => ({
      S_title,
      S_description,
    }));
    formData.append('sections', JSON.stringify(sectionMeta));
